...
package com.ru.etcs.media {
	...
	public class WaveSound extends EventDispatcher {
		...
		*/
		 ...
		 */
		public function WaveSound( audioData:ByteArray, stream:URLRequest = null ) {
			super();
			...
			if (stream != null) {
				...
				//trace( "WaveSound | audioData.length=" + audioData.length );
				loadAudio( audioData );
			}
		}
		...
		*/
		...
		private function loadAudio( audioData:ByteArray ):void{
			waveHeader = new ByteArray();
			waveHeader.endian = Endian.LITTLE_ENDIAN;
			waveData = new ByteArray();
			waveData.endian = Endian.LITTLE_ENDIAN;
			audioData.readBytes( waveHeader, 0, PCMFormat.HEADER_SIZE );
			waveFormat = new PCMFormat();
			...
			var bytesToRead:uint = audioData.bytesAvailable < waveFormat.waveDataLength ? audioData.bytesAvailable : waveFormat.waveDataLength;
			audioData.readBytes( waveData, 0, bytesToRead );
			var swf:SWFFormat = new SWFFormat( waveFormat );
			var compiledSWF:ByteArray = swf.compileSWF(waveData);
			var loader:Loader = new Loader();
			loader.loadBytes(compiledSWF,loaderContext);
		...
		*/
		...
		{
			var num:Number = 0;
			num = sound.extract( target, length, startPosition );
		...
		*/
		...
		/**
		 * The state of loading complied swf. read-only. 
		 */
		 ...
		 * Return the sound
		 */
		public function get customSound():Sound{
			return sound;
		}
	}
